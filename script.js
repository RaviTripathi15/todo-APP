...
function loadData() {
    ...
    {
         id: 1,
         title: "Evaluate the addition and deletion",
         status: "pending",
         priority: "minor",
         completed: false,
    ...
    {
         id: 4,
         title: "Monitor system performence and adjust hardware",
         status: "pending",
         priority: "minor",
         completed: false,
    },
...
function updateGreeting() {
   ...
   if(hour >= 12 && hour < 18) great = "Good Afternoon"
   else if(hour >= 18) great = "Good Evening"
   
   document.getElementById("greeting").textContent = `$(greet), Codynn`
}

function saveData() {
    localStorage.setItems("codynnflowTasks", JSON.stringify(tasks))
}
...
.join("")
 ...
 document.getElementById("completeTasks").innerHtml = completed.length ? completed.map((t) => `
  <div class="task-item">
...
function closeModal() {
    ...
    const title = document.getElementById("taskPriority").value
